...
app.get("/", function (req, res) {
  ...
  browserJSON.ipaddress = ip.split(',')[0];
  browserJSON.language = req.headers["accept-language"].split(',')[0];
  var operatingSystem = req.headers['user-agent'].match(/\(([^)]+)\)/)[1]
  browserJSON.software = operatingSystem; 
  res.send(browserJSON);
});
app.listen(process.env.PORT);